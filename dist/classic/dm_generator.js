...
var MAG_SPECS = {
  ...
  SD: 'SD|代号|出手风格|一句圈内风评 —— 4条慷慨榜（可混入 纪司柏/顾维 等已知人物+新面孔），风评=姐妹们的实用情报+调侃\n',
...
async function generateMagOnce(sb, plot, strict, onlyKeys) {
  ...
  if (keys.indexOf('ABYSS') !== -1) {
    ...
    ordered.push({ role: 'system', content: '【这一版意淫的对象是谁】\n'
      + (whoA.length ? whoA.join('\n')
         : '玩家没填人设——那就只从主线剧情里她被描写过的样子、穿过的、去过的地方、跟谁走了这些**已经发生过的**细节里取素材，绝不自己发明一套长相。')
  ...
  if (sb.game && sb.game.random_only) {
    ...
    instr = instr.replace('（可混入 纪司柏/顾维 等已知人物+新面孔）', '（全部用新面孔）');
