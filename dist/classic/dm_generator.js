...
var ANON_GROUP_NAME = '🎭 化妆间';            // 化妆间＝高级场所里女客补妆的那间屋子，门一关谁都敢说真话
...
function mustSpeakers(order, text) {
  ...
  for (var i = 0; i < (order || []).length; i++) {
    ...
    var parts = String(nm).split(/\s+/);                       // 名姓分开写的（Marco Rossi）：玩家多半只写前半截
...
function capGroupSpeakers(rows, k, musts) {
  ...
  for (i = 0; i < rows.length; i++) {
    ...
    console.warn('[SB-S v4] 这一轮只有 ' + k + ' 个人说话，多出来的行丢弃：' + r.who);
  }
  return out;
}
...
function migrateGroups() {
  updateVariablesWith(function (v) {
    ...
    if (npcs[GROUP_NAME] && !npcs[GROUP_NAME].isGroup) {
      ...
      console.log('[SB-S v4] migrated legacy group: ' + GROUP_NAME);
    }
    for (var k in npcs) {
...
function pushThem(sb, name, type, content, zh, delay, who) {
  ...
  var pfx = who ? who + '：' : '';   // 群列表预览要带说话人（"纪司柏：在吗"），不然一串话不知道谁说的
...
function describeState(sb) {
  ...
  // 玩家人设整段给（2026-09-16 拔掉 1200 字截断：人设写得越细越长，砍掉的恰恰是后面那些具体特征）
...
function parseDMs(raw, groupCtx) {
  ...
  // 模型漏写群名里的 emoji/空格（"化妆间"、"🎭化妆间"）→ 折叠后对上就算同一个群，别建分身
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    var twoOk = parts.length === 2 && (t === 'block' || t === 'unblock');   // 「纪司柏|block」不带尾巴的竖线也认（拉黑行内容本来可空）
  ...
  for (var j = 0; j < rows.length; j++) {
    ...
    if (gName) {
      if (GROUP_TYPES.indexOf(rows[j].type) === -1) {   // 钱类/标签/行程/拉黑落到群行上 → 丢（钱只走私信一个口子）
        console.warn('[SB-S v4] 群行类型不合法，丢弃：' + gName + '|' + rows[j].type);
        continue;
      }
      ...
      if (rows[j].type === 'leave') {   // 退群行的内容本身就是那个人（可能还跟了句气话）
        ...
        if (!lw) { console.warn('[SB-S v4] 群里冒出名单外的名字「' + content + '」，这一行丢了'); continue; }
        out.push({ name: gName, type: 'leave', content: '', zh: '', who: lw, group: gName, delay: 0 });
        continue;
      }
      if (!sp.who) {   // 名单外的名字，或整行没写说话人 → 整行丢（说错人比说不好更糟）
        console.warn('[SB-S v4] 群行的说话人不在名单里，丢弃：' + gName + '|' + rows[j].type + '|' + String(content).slice(0, 40));
        continue;
      }
    ...
    if (rows[j].type === 'sticker' && NO_STICKER.test(gName ? who : rows[j].name)) content = '';   // 不发表情包的人（群里也一样）——代码闸，提示词管不住的这里管
...
function groupFormatRule(g, order, dmHint, k, musts) {
  ...
  var s = '【输出格式】每条消息占一行，格式严格为：\n' +
    ...
    '- 每条 text/image/voice 的内容末尾以§收尾：群里基本都说中文，§后留空；偶尔有一条是英文（或其他外语）的，§后写它的中文翻译（人名/地名/品牌保留英文）\n' +
...
// 群里的 User 是谁：熟人群给玩家自己填的人设（本卡没有建档页，唯一可信来源是酒馆人设，同 describeState）；
// 匿名群只给代号（群里的人本来就只看得到代号）
...
async function generateGroupOnce(sb, plot, n, reason, strict, groupName, plan) {
  ...
  ordered.push({ role: 'system', content:
    ...
    '世界：2026 年 S 市，圈内邀请制的私信 App（上了平台的人都知道彼此是干嘛的），群聊也长在这个 App 里。\n' +
...
async function generateOnce(sb, plot, n, reason, strict, group, plan) {
  ...
  var sys1 =
    ...
    '- 表情包：名字|sticker|表情包名 —— 名字必须一字不差取自【表情包清单】，它是一条独立消息（不带§、不带别的字），一轮最多一两张。谁爱发看人设：年轻的/话痨的/装可爱的/同行姐妹爱发，惜字如金的老钱不发；表情包的情绪要和上下文对得上\n' +
...
function openAnonDm(sb, gKey, handle, firstLine, zh) {
  ...
  if (!r || !r.real) { console.warn('[SB-S v4] 这个代号还没露过底，dm 行丢弃：' + handle); return ''; }
...
async function runOnce(req) {
  ...
  function route(arr) {
    for (var i = 0; i < arr.length; i++) {
      ...
      if (r.group) { console.warn('[SB-S v4] 普通私信轮里冒出群行，丢弃（群有自己的生成管道）：' + r.group); continue; }
  ...
  console.log('[SB-S v4] generated ' + (isGroupReq ? (grows.length + ' group rows + ' + sideN + ' side DMs') : (dms.length + ' DMs')));
  if (anonDmName) {
    try { if (typeof toastr !== 'undefined') toastr.info('💌 群里有人私下加了你：' + anonDmName, 'SugarOS'); } catch (e) {}
    console.log('[SB-S v4] anon group DM → ' + anonDmName);
  }
  ...
  if (floorLogOn()) { try { await appendPhoneLog(logRows); } catch (e) { console.warn('[SB-S v4] append phone log failed', e); } }
}
...
function buildDigest(sb) {
  ...
  if (myPostsB.length) {
    out += '【User 用马甲在 SugarSecret 论坛发过的帖子（圈内公开可见）】' + myPostsB.map(function (p) { return '「' + String(p.text || '') + '」'; }).join('；') + '\n' +
...
async function maybeUnlockAnonGroup() {
  ...
  console.log('[SB-S v4] anon group unlocked at floor ' + floors + ' (handle ' + myHandle + ')');
  try {
    handleRequest({
...
async function maybeAutoStranger() {
  ...
  if (gPick) {
    ...
    try {
      ...
      console.log('[SB-S v4] auto-slot spent on group ambience: ' + gPick + ' (turn ' + turns + ')');
    } catch (e) {}
    return;
  }
  try {
...
async function onMainMessage(message_id) {
  try {
    ...
    if (!isSwipe) {
      try { await maybeUnlockAnonGroup(); } catch (e) { console.warn('[SB-S v4] anon group unlock check failed', e); }   // 🎭 聊满 50 楼 → 管家拉你进匿名大厅（只炸一次）
    ...
    if (!isSwipe) {
      try {
        if (/(?:论坛|SugarRank|sugar\s*rank)[^。\n]{0,30}(?:发[了个]?帖|发了一[个条]|帖子|贴子|发贴)|(?:招聘|召集|征人|找金主|求包养)[^。\n]{0,20}(?:帖|贴)/.test(text)) {
          ...
          console.log('[SB-S v4] forum post detected → firing ' + forumN + ' auto-DMs');
        }
      } catch (e) { console.warn('[SB-S v4] forum detect failed', e); }
    }
    // ① 钱包标记
...
async function generateMagOnce(sb, plot, strict, onlyKeys) {
  ...
  if (keys.indexOf('ABYSS') !== -1) {
    ...
    // 原文整条给（2026-09-16 拔掉 60 字截断和补的"…"：只看半句，模型认不出自己用过的梗和身体部位，照样撞车）
    var prevA = ((sb.mag && sb.mag.abyss) || []).map(function (x) { return String(x.body || ''); }).filter(Boolean);
...
// ── 🔭 跨卡足迹彩蛋（祈星专属，100楼+200楼各触发一次，共两次）──
