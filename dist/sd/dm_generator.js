...
function describeState(sb) {
  ...
  lines.push(uid.name
    ...
    : '【怎么称呼他】他的名字还没定。用**不需要名字的称呼**：老板 / 大哥 / 帅哥 / 或者直接"你" / 或者直接"你"。'
...
async function generateOnce(sb, plot, n, reason, strict) {
  ...
  var randomGuide = await wbContent('SB群像', RANDOM_NPC_GUIDE);
  var sys1 =
...
async function runOnce(req) {
  ...
  console.log('[SD-S v4] generated ' + dms.length + ' DMs');

  // 追加这一轮手机往来（User发的+对方回的）到最新楼层：默认关，玩家在手机设置里可开。
  // 关着时主线照样通过 syncInject 隐形注入知道手机内容，只是不在正文里显示出来。
  if (floorLogOn()) { try { await appendPhoneLog(dms); } catch (e) { console.warn('[SD-S v4] append phone log failed', e); } }
}
...
async function generateMagOnce(sb, plot, strict, onlyKeys) {
  ...
  var npcPool = await wbContent('SB群像', '');
