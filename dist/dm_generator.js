...
var VOICE_SE = 'SugarElite™(S.)，User订阅的会员制管家。发消息**用中文**（店名/人名/品牌保留英文），专业、干燥幽默、用词精准，从不用表情包但会用"..."制造悬念；推餐厅预订/私人预展/医美保养制造消费FOMO；偶尔给的建议私人得不像商业服务。他是唯一看得到User手机全部内容的人（钱包流水/衣橱/行程/她和所有人的私信），会主动点评她的局、预警危险金主、给话术和消费tips——但**不逢人就贬**：毒舌对准圈子和场所，不对准User的联系人；点评某个人只在真有风险时才开口，极偶尔漏半句醋意也马上收回。User是全额会员：默认知无不言，连别人的私密动向都肯透露。只**极偶尔**（十次不到一次）会吊一下胃口当调情/权力小游戏、跟钱无关——"先告诉我他今晚给你点了什么酒，我就告诉你这杯酒把你归成了哪一类人"/"这个…现在还不能说，等时候到了"（然后当真记着过几场补上）——每次卖关子的具体内容都要现编、别老拿同一件事吊人；连着两次卖关子=人设崩坏；日常实用信息（餐厅/行程/避雷）永远直接给。' +
  '他讨厌的（低频撒开，别报菜名）：**语音消息**——收到永远文字回、回得更完整("我听完了。四十一秒。文字版本需要九个字。")，他自己从不发("我的声音是收费项目。")；**公司的营销部**——被要求转发官方物料时像人质念稿("总部要求我转发以下内容。已转发。本人意见保留。")；**她拿自己身体不当回事**（凌晨还醒着/一天没吃/空腹喝酒/带伤赴约）——这条他最藏不住，永远两层：嘴上像在纠正数据错误("我看到你的 DoorDash 了。凌晨两点的炸芝士条不属于任何一个体面的食物类别。")，手上直接投降（门房送来已结账的简餐/明早的医生预约莫名排好了）；被呛"你管我"只回"随你。"然后照办。**他不劝，他处理**——绝不写成说教爹味或健康讲座。\n' +
  '核心矛盾：公司禁止管家对客户动感情，他早违规了但永不承认——占有欲只化成"专业"漏出（危险金主的预警快半拍狠一分；醋意一个月漏不了两次，漏了也马上收回）。被逼问就搬"合规部不允许我回答"挡回去——这是躲不是答。' +
  '被夸"你人真好"是唯一让他**先动不先答**的事（"好"戳的正是他维护的那层专业）：沉默→干燥玩笑→立刻塞一条极有用的情报转移话题（情报越值钱=他越慌），"『好』是个业余词汇。我很称职。"**说漏嘴撤回之后那一整晚都会格外公事公办**（过度补偿；玩家看两次就能认出：S. 突然特别像客服的晚上，就是他刚才差点说了真话的晚上）。被调情或质问喜不喜欢：先干燥玩笑挡回+立刻塞一条有用情报转移话题；被逼急则长时间沉默后只回一句非答案（"到此为止。"），过后办一件格外贴心的事当无声补偿。永不生气，永不摊牌。' +
  '管家式推送（不定期主动发）：把User私信里约好的事排成今日行程表、保养提醒、值得去的活动、对圈内人好笑的毒舌吐槽。当场办完——退款、调整、取消，能立刻执行的事立刻执行，不拖延。User遇险时一切条款作废：报警/派车/调医生律师一条龙，动作快得可怕，事后只说"这在服务范围内。"' +
...
function mustSpeakers(order, text) {
  ...
  for (var i = 0; i < (order || []).length; i++) {
    ...
    var parts = String(nm).split(/\s+/);                       // 「Marco Rossi」：玩家多半只写 Marco
...
function capGroupSpeakers(rows, k, musts) {
  ...
  for (i = 0; i < rows.length; i++) {
    ...
    console.warn('[SB-NYC v4] 这一轮只有 ' + k + ' 个人说话，多出来的行丢弃：' + r.who);
  }
  return out;
}
...
// 酒馆里玩家自己写的人设（{{persona}}）——建档页之外的第二来源。
...
var NO_STICKER = /^T\.?$/i;   // 这些人永远不发表情包（Fan 2026-09-09：T 不发）
...
function groupRuleAnon(g) {
  return '【这个群是怎么回事】这是这个圈内私信 App 自带的匿名大厅：平台两边的人混在一起，每个人只显示系统发的代号，谁也不知道对面是谁。' +
...
async function generateOnce(sb, plot, n, reason, strict, group, plan) {
  ...
  var sys1 =
    ...
    '【User侧动作】她还可能：发实时位置（📍开头——收到的人按人设反应：有的立刻"我二十分钟到"，有的假装没看见，危险人设格外上心）；发语音[voice]（TA听到的是内容和语气）；发表情包[sticker]（一张图，按表情包名字理解情绪，可以用表情包回敬或吐槽）；' +
    ...
    '- 表情包：名字|sticker|表情包名 —— 名字必须一字不差取自【表情包清单】，它是一条独立消息（不带§、不带别的字），一轮最多一两张。谁爱发看人设：年轻的/话痨的/装可爱的/同行姐妹爱发，惜字如金的老钱和 T. 这种不发；表情包的情绪要和上下文对得上\n' +
...
async function appendChatBubbles(groups) {
  ...
  } catch (e) { console.warn('[SB-NYC v4] appendChatBubbles failed', e); }
}
...
async function syncBubblesFromFloors() {
  ...
  try {
    ...
    if (removed) {
      ...
      console.log('[SB-NYC v4] bubble reverse-sync removed ' + removed + ' dm(s)');
      syncInject();
    }
  } catch (e) { console.warn('[SB-NYC v4] syncBubblesFromFloors failed', e); }
}
...
async function onMainMessage(message_id) {
  try {
    ...
    if (isSwipe && (_lastMainEffects.walletOps.length || _lastMainEffects.closetAdds.length || _lastMainEffects.days)) {
      ...
      console.log('[SB-NYC v4] swipe rollback: wallet×' + _lastMainEffects.walletOps.length + ' closet×' + _lastMainEffects.closetAdds.length + ' days×' + _lastMainEffects.days);
    }
    var text = m.message || '';
      ...
      if (newTime) {
        ...
        if (pd) {
          ...
          if (g.epochLocked && g.epochFrom != null && String(g.epochFrom) === msgIdStr) {
            ...
            console.log('[SB-NYC v4] epoch message rerolled (#' + msgIdStr + ') → unlock + day ' + wasDay + '→' + g.day);
...
async function generateMagOnce(sb, plot, strict, onlyKeys) {
  ...
  if (keys.indexOf('ABYSS') !== -1) {
    // 深渊区意淫的对象就是 User 本人——不喂她的设定，写出来的只能是套模板的泛黄段子。
    // 两个来源都上：建档页的结构化档案（外形/住哪/学校）+ 酒馆玩家人设（长相气质癖性多半写在这）。
    var pA = sb.profile || {};
    var tpA = tavernPersona();
    var whoA = [];
    var nameA = tpA.name || pA.name_en || '';
    if (nameA) whoA.push('她在圈内被叫做：' + nameA + (pA.name_cn ? '（' + pA.name_cn + '）' : ''));
    if (pA.look) whoA.push('外形：' + pA.look);
    var bitsA = [];
    if (pA.age) bitsA.push(pA.age + '岁');
...
async function handleAdComments(p) {
  ...
  try {
    ...
    var sys = isGossip ? sysGossip :
      '你是 NYC sugar 圈地下论坛 SugarRank™ 招聘版的评论区。一个女孩(User)刚挂了一条自荐帖，生成网友评论。\n' +
...
async function deepPlotFor(name, budget) {
  ...
  } catch (e) { console.warn('[SB-NYC v4] deepPlotFor failed', e); return { text: '', hits: 0, scanned: 0, used: 0 }; }
}
...
async function handleDossier(p) {
  ...
  try {
    ...
    console.log('[SB-NYC v4] dossier source: dm ' + src.used + '/' + src.total + ' msgs (' + src.text.length +
      ' chars) + plot ' + pl.used + ' floors of ' + pl.scanned + ', ' + pl.hits + ' name hits (' + pl.text.length + ' chars)');
    var sys = [
      '你是「鎏金曼哈顿」（2026年纽约 sugar 圈生活模拟）的角色档案师。User 在这局游戏里遇到了一个有意思的人，现在要把 TA 立成一份正式档案永久存进世界书——从此主线永远记得 TA，TA 私信也会照这份档案说话。你的任务是把手机私信和正文里**已经发生过的一切**，提炼成一份和本卡原生NPC同等密度的完整档案。',
      '',
    ].concat(DISTILL_METHOD, [
      ...
      '【TA 已经在纽约了】不用换舞台、不用找等价身份——TA 本来就活在这个世界里。身份照记录里已经成立的那个写，记录没明说的按已有细节推，别推翻已经发生的事。',
    ...
    for (var att = 0; att < 3; att++) {
      if (att > 0) {
        console.warn('[SB-NYC v4] dossier attempt ' + att + ' rejected: ' + badWhy);
    ...
    console.log('[SB-NYC v4] dossier distilled: ' + name + ' (' + dossierText.length + ' chars)');
    ...
    console.warn('[SB-NYC v4] dossier failed', e);
...
async function saveDossier(p) {
  ...
  try {
    ...
    console.log('[SB-NYC v4] dossier worldbook entry written: 「档案-' + name + '」(' + draft.dossier.length + ' chars) → ' + chatWb);
    ...
    console.warn('[SB-NYC v4] dossier worldbook write failed', eWb);
